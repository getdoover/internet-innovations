...
device_msg_id_key = "SerNo" # the key for the value in the payload that identifies the device
device_config_id_key = "II_SERIAL" # the key in the deployment config that contains the identifier of the device
publish_channel = "ii_oem_uplink_recv" # the channel messages are published to
...
class target:
    ...
    def process_connector_message(self, msg_obj):
        ...
        for ak, a in agents.items():
            ...
            self.add_to_log("deployment config is " + str(deployment_config)+ "| type is: " + str(type(deployment_config)))
            self.add_to_log("device_config_id_key is "+ str(device_config_id_key)+"| type is: "+ str(type(device_config_id_key)))
